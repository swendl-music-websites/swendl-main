...
function _populate() {
    if (!_populated) {
        ...
        if (((_iphone = /\b(iPhone|iP[ao]d)/.exec(t)), (_ipad = /\b(iP[ao]d)/.exec(t)), (_android = /Android/i.exec(t)), (_native = /FBAN\/\w+;/i.exec(t)), (_mobile = /Mobile/i.exec(t)), (_win64 = !!/Win64/.exec(t)), e)) {
            ...
            (_ie_real_version = s ? parseFloat(s[1]) + 4 : _ie),
                ...
                (_webkit = e[4] ? parseFloat(e[4]) : NaN) ? ((e = /(?:Chrome\/(\d+\.\d+))/.exec(t)), (_chrome = e && e[1] ? parseFloat(e[1]) : NaN)) : (_chrome = NaN);
...
function isEventSupported(t, e) {
    ...
    return !s && useHasFeature && "wheel" === t && (s = document.implementation.hasFeature("Events.wheel", "3.0")), s;
}
...
function normalizeWheel(t) {
    ...
    return (
        ...
        (s || n) && t.deltaMode && (1 == t.deltaMode ? ((s *= LINE_HEIGHT), (n *= LINE_HEIGHT)) : ((s *= PAGE_HEIGHT), (n *= PAGE_HEIGHT))),
        s && !e && (e = s < 1 ? -1 : 1),
        n && !i && (i = n < 1 ? -1 : 1),
...
class ClapatSlider {
        ...
        (this.opts = Object.assign(
            {
                ...
                renderBullet: null,
                parallax: null,
                on: { init: null, activeSlideChange: null, slideEnterViewport: null, slideLeaveViewport: null },
            },
            e
        )),
    ...
    setupEvents() {
        ...
        null != a && (a.addEventListener(i, this.onDown), a.addEventListener(t, this.onMove), this.opts.mousewheel && (a.addEventListener(n, this.onWheel), a.addEventListener(l, this.onWheel)), window.addEventListener(e, this.onUp)),
    ...
    off() {
        ...
        let a = this.elEventTarget;
        null != a &&
            (a.removeEventListener(i, this.onDown), a.removeEventListener(t, this.onMove), this.opts.mousewheel && (a.removeEventListener(n, this.onWheel), a.removeEventListener(l, this.onWheel)), window.removeEventListener(e, this.onUp)),
    ...
    setup() {
        ...
        for (let i = t.length - 1; i >= 0; i--) {
            ...
            "vertical" == this.opts.direction
                ? ((s.style.top = 100 * i + "%"), (n.style.top = 100 * -e + "%"), n.classList.add(this.opts.clones), this.elInner.append(n))
                : ((s.style.left = 100 * i + "%"), (n.style.left = 100 * -e + "%"), n.classList.add(this.opts.clones), this.elInner.prepend(n)),
                e++;
        }
        ...
        for (let t = 0; t < i.length; t++) {
            ...
            let o = null;
            "vertical" == this.opts.direction ? ((o = { el: e, start: l, end: a, length: r, translate: 0 }), (this.length += r)) : ((o = { el: e, start: s, end: n, length: h, translate: 0 }), (this.length += h)),
                e.classList.contains(this.opts.clones) ? (this.itemsCloned.push(o), (o.clone = !0)) : (this.itemsInitial.push(o), (o.clone = !1)),
                this.items.push(o);
        }
        ...
        if ("vertical" == this.opts.direction) {
            for (let t = 0; t < s; t++) {
                ...
                (e.prevElement = 0 == t ? this.items[s] : this.items[t - 1]), (e.nextElement = t == s - 1 ? this.items[this.items.length - 1] : this.items[t + 1]);
            }
            for (let t = this.items.length - 1; t >= s; t--) {
                const e = this.items[t];
                t == this.items.length - 1 ? (e.prevElement = this.items[s - 1]) : (e.prevElement = this.items[t + 1]), (e.nextElement = t == s ? this.items[0] : this.items[t - 1]);
            }
        } else
            for (let t = 0; t < this.items.length; t++) {
                const e = this.items[t];
                (e.prevElement = 0 == t ? this.items[this.items.length - 1] : this.items[t - 1]), t == this.items.length - 1 ? (e.nextElement = this.items[0]) : (e.nextElement = this.items[t + 1]);
            }
        null != this.el && 0 != this.opts.click && this.el.addEventListener("click", this.onClick), gsap.ticker.add(this.tick);
    }
    tick() {
        this.render();
    }
    calc() {
        const t = this.state;
        (t.current += (t.target - t.current) * this.opts.ease), (t.currentRounded = Math.round(100 * t.current) / 100), (t.diff = t.target - t.currentRounded);
        const e = Math.round((-this.length / 2) * 100) / 100;
        (t.progress = gsap.utils.wrap(0, 1, t.currentRounded / e)), (t.moveOffset = gsap.utils.wrap(0, this.length, Math.abs(t.currentRounded))), this.tl && this.tl.progress(t.progress);
    }
    render() {
        this.state.flags.resizing || (this.opts.snap && this.updateWheelSnap(), this.calc(), this.transformItems(), this.parallax());
    }
    transformItems() {
    ...
    updateUI() {
        ...
        for (let a = 0; a < this.items.length; a++) {
            ...
            let r = s * n;
            r < 0 && h.end + r < t + e + h.length - this.length && (r += this.length), r > 0 && h.end + r > this.length && (r -= this.length);
            ...
            if (
                ...
                t && !e && "function" == typeof this.opts.on.slideLeaveViewport && this.opts.on.slideLeaveViewport(h.el), !t && e && "function" == typeof this.opts.on.slideEnterViewport && this.opts.on.slideEnterViewport(h.el);
            }
        }
        this.state.flags.dragging ? this.el.classList.add("clapat-state-dragging") : this.el.classList.remove("clapat-state-dragging"), this.updatePaginationUI(), null == l && "function" == typeof this.opts.on.init && this.opts.on.init();
    }
    updatePaginationUI() {
        if (this.opts.pagination && !(this.itemsPagination.length <= 0) && null != this.state.currentSlideItem) {
            ...
            for (let t = 0; t < this.itemsPagination.length; t++) {
                ...
                t != e && i.el.classList.remove("clapat-pagination-bullet-active"), t != l && i.el.classList.remove("clapat-pagination-bullet-next"), t != s && i.el.classList.remove("clapat-pagination-bullet-prev");
            }
        }
    }
    parallax() {
        ...
        const t = this.viewportSize(),
            e = this.viewportShift(),
            i = (this.elInner.getBoundingClientRect(), this.state.moveOffset * Math.sign(this.state.currentRounded));
        for (let s = 0; s < this.items.length; s++) {
            const n = this.items[s];
            let l = i;
            if ((l < 0 && n.end + l < t + e + n.length - this.length && (l += this.length), l > 0 && n.end + l > this.length && (l -= this.length), n.end + l > e && n.start + l < t + e)) {
    ...
    snapTargetOnDrag(t) {
        ...
        for (let l = 0; l < this.items.length; l++) {
            ...
            let h = s * n;
            h < 0 && a.end + h < e + i + a.length - this.length && (h += this.length), h > 0 && a.end + h > this.length && (h -= this.length);
    ...
    snapTargetOnWheel(t, e) {
        ...
        for (let t = 0; t < this.items.length; t++) {
            let e = this.items[t],
                h = n * l;
            h < 0 && e.end + h < i + s + e.length - this.length && (h += this.length), h > 0 && e.end + h > this.length && (h -= this.length);
        ...
        for (let n = 0; n < this.items.length; n++) {
            let l = this.items[n],
                o = h * r;
            o < 0 && l.end + o < i + s + l.length - this.length && (o += this.length), o > 0 && l.end + o > this.length && (o -= this.length);
            ...
            if (Math.abs(c) < l.length / 2) {
                return a == n && Math.abs(e - t) < this.length
                    ? (this.log("Snapping - start and end items are the same, the scroll did not go over a full item"),
                      this.state.currentScrollDirection > 0 ? (c < 0 ? e - (l.length / 2 + c + l.nextElement.length / 2) : e - c) : c < 0 ? e - c : e - c + l.prevElement.length)
    ...
    onMove(t) {
        ...
        Math.abs(a) > Math.abs(h) && t.cancelable && t.preventDefault(),
            "vertical" == this.opts.direction ? ((e.target = n + h * this.opts.speed), (e.currentScrollDirection = -Math.sign(h))) : ((e.target = n + a * this.opts.speed), (e.currentScrollDirection = -Math.sign(a)));
    }
    onUp(t) {
        if (!this.enabled) return;
        this.log("on up");
        ...
        if (((e.flags.mousedown = !1), (e.flags.dragging = !1), (e.off = e.target), i)) {
            if (((e.flags.click = !1), t.cancelable)) return t.preventDefault(), t.stopPropagation(), this.log("ending dragging"), !1;
    ...
    onClick(t) {
        if (this.enabled) {
            if ((this.log("on click. Drag? " + !this.state.flags.click), this.state.flags.click)) {
                let e = null;
                null != t.target && ((e = t.target.querySelector("a")), null != e && e.click());
            }
    ...
    isItemInsideView(t, e) {
        let i = 0,
            ...
            l = e.end + e.translate;
        return "vertical" == this.opts.direction ? ((i = t.top + 5), (s = t.bottom - 5)) : ((i = t.left + 5), (s = t.right - 5)), l > i && n < s;
    }
    distanceToCenter(t) {
    ...
    update() {
        ...
        for (let l = 0; l < this.items.length; l++) {
            const a = this.items[l];
            let h = i * s;
            h < 0 && a.end + h < t + e + a.length - this.length && (h += this.length), h > 0 && a.end + h > this.length && (h -= this.length);
    ...
    onPrev(t) {
        ...
        for (let t = 0; t < this.items.length; t++) {
            ...
            let a = s * n;
            a < 0 && l.end + a < e + i + l.length - this.length && (a += this.length), a > 0 && l.end + a > this.length && (a -= this.length);
            const h = l.start + a - (i + (e - l.length) / 2);
            Math.abs(h) < l.length / 2 && ((this.state.target = this.state.target - h + l.prevElement.length), (this.state.off = this.state.target));
        }
    }
    onNext(t) {
        if (!this.enabled) return;
        ...
        for (let t = 0; t < this.items.length; t++) {
            ...
            a < 0 && l.end + a < e + i + l.length - this.length && (a += this.length), a > 0 && l.end + a > this.length && (a -= this.length);
            const h = l.start + a - (i + (e - l.length) / 2);
            Math.abs(h) < l.length / 2 && ((this.state.target = this.state.target - (l.length / 2 + h + l.nextElement.length / 2)), (this.state.off = this.state.target));
        }
    }
    onWheel(t) {
        if (!this.enabled) return;
        ...
        i.flags.dragging = !1;
        const { off: a, on: h } = i;
        if ((this.log("Event timestamp: " + e + " Scroll delta " + s), this.opts.snap && null != this.snapWheelEvents.tsSnap)) {
            if (e - this.snapWheelEvents.tsSnap <= this.opts.snapwait.after) return void this.log("Ignoring wheel event as there is a snapping going on");
        ...
        (i.target -= s * this.opts.speed),
            ...
            this.opts.snap && this.snapWheelEvents.events.push({ ts: e, delta: s, currentTarget: i.target }),
    ...
    updateWheelSnap() {
        if (this.opts.snap && this.snapWheelEvents.events.length > 0) {
            ...
            if (t - this.snapWheelEvents.events[this.snapWheelEvents.events.length - 1].ts > this.opts.snapwait.before) {
                ...
                const r = this.state,
                    { off: o, on: c } = r;
                (r.target = this.snapTargetOnWheel(h, r.target)), "vertical" == this.opts.direction ? (c.y = r.target) : (c.x = r.target), (r.off = r.target), (this.snapWheelEvents.tsSnap = t), (this.snapWheelEvents.events = []);
            }
        }
    }
    debounce(t, e) {
